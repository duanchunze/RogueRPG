...
namespace Hsenl {
    ...
    public partial class Numerator : Unbodied, INumerator {
        public static uint MaxLayer { get; private set; }

        public static void InitNumerator(int maxLayer) {
            MaxLayer = (uint)maxLayer;
        }

        public static void InitNumerator<T>(T maxLayer) where T : Enum {
            MaxLayer = (uint)maxLayer.GetHashCode();
        }
        ...
        public bool IsHasValue(uint numericType) {
            if (numericType is < 1 or > NumericConst.NumericTypeMax)
                throw new ArgumentOutOfRangeException($"num key must be range of 1 - {NumericConst.NumericTypeMax} '{numericType}'");
        ...
        public (Num fix, Num pct) GetAttachValue(uint numericType, uint layer) {
            ...
            if (layer is < 1 or > NumericConst.NodeLayerMax) {
                throw new ArgumentOutOfRangeException($"num layer must be range of 1 - {NumericConst.NodeLayerMax} '{layer}'");
            }
        ...
        public Num GetFinalValue(uint numericType) {
            if (numericType is < 1 or > NumericConst.NumericTypeMax)
                throw new ArgumentOutOfRangeException($"num key must be range of 1 - {NumericConst.NumericTypeMax} '{numericType}'");
            return !this._finalNumerics.TryGetValue(numericType, MaxLayer - 1, out var result) ? Num.Empty() : result;
        }
        ...
        private void RecalculateFinalNumeric(uint numericType, bool sendEvent = true) {
            ...
            this._finalNumerics.TryGetValue(numericType, MaxLayer - 1, out var old);
            for (uint layer = 1; layer < MaxLayer; layer++) {
                var pctKey = new NumericNodeKey(numericType, layer, NumericNodeModel.Percentage);
                var fixKey = new NumericNodeKey(numericType, layer, NumericNodeModel.FixedValue);
