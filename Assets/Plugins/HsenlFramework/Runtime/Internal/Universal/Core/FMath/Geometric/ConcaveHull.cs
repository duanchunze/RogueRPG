...
namespace Hsenl {
    ...
    public static class ConcaveHull {
        public static bool Create2D(Vector2[] points, out int[] concaveHull, out int[] convexHull,
        ...
        public static bool Create2D(Vector2[] points, out int[] concaveHull, FLOAT algorithmThreshold) {
            FLOAT epsilon = 1E-05f;
            return Create2D(points, out concaveHull, out _, algorithmThreshold, epsilon);
        }

        public static bool Create2D(Vector2[] points, out int[] concaveHull, FLOAT algorithmThreshold, FLOAT epsilon) {
            return Create2D(points, out concaveHull, out _, algorithmThreshold, epsilon);
        }
    }
    ...
    internal class ConcaveHull2 {
        ...
        private static FLOAT CalcDistanceFromPointToEdge(ref Vector2 pointA, ref Vector2 v0, ref Vector2 v1) {
            ...
            return Math.Abs(f) / Math.Sqrt(num5);
        }

        public static bool Create(Vector2[] points, out int[] concaveHull, int[] convexHull, FLOAT N) {
            // FLOAT epsilon = 1E-05f;
            ...
            while (linkedListNode != null && hashSet.Count != 0) {
                ...
                Vector2 v3 = points[v];
                Vector2 v4 = points[v2];
                int num6 = 0;
                foreach (int item in hashSet) {
                    Vector2 vector = points[item];
                    FLOAT num7 = vector.x - v3.x;
                    FLOAT num8 = vector.y - v3.y;
                    FLOAT num9 = Math.Sqrt(num7 * num7 + num8 * num8);
                    num7 = vector.x - v4.x;
                    num8 = vector.y - v4.y;
                    FLOAT num10 = Math.Sqrt(num7 * num7 + num8 * num8);
                ...
                for (int num11 = num6; j < num11; j++) {
                    ...
                    Vector2 pointA = points[innerPoint3.Index];
        ...
        public static bool Create(Vector2[] points, out int[] concaveHull, int[] convexHull, FLOAT N, FLOAT epsilon) {
            LinkedList<Edge> linkedList = new LinkedList<Edge>();
            int num = convexHull.Length;
            HashSet<int> hashSet = new HashSet<int>();
            int num2 = 0;
