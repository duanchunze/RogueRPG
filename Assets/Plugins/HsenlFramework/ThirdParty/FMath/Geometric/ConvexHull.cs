...
namespace FixedMath {
    ...
    public static class ConvexHull {
        ...
        public static bool Create2D(IList<FVector2> points, out int[] indices, out int dimension, FLOAT epsilon) {
        ...
        /// <param name="epsilon">Small positive number used to determine dimension of the input set.</param>
        /// <returns>True if convex hull is created, false otherwise (in case if input point set is null, contains no points or if some error has occured during construction)</returns>
        public static bool Create3D(IList<FVector3> points, out int[] indices, out int dimension, float epsilon = 1E-05f) {
            if (points == null || points.Count == 0) {
                indices = null;
                dimension = -1;
                return false;
    ...
    internal class ConvexHull2 {
        ...
        public static bool Create(IList<FVector2> vertices, FLOAT epsilon, out int dimension, out int[] indices) {
            ...
            if (information.Dimension == 1) {
                ...
                FVector2 origin = information.Origin;
                FVector2 vector = information.Direction[0];
                for (int i = 0; i < count; i++) {
                    FVector2 value = vertices[i] - origin;
            ...
            dimension = 2;
            FVector2[] array3 = new FVector2[count];
            FVector2 min = information.Min;
            FLOAT num = 1f / information.MaxRange;
            for (int j = 0; j < count; j++) {
                ref FVector2 reference = ref array3[j];
    ...
    internal class ConvexHull3 {
        ...
        public static bool Create(IList<FVector3> vertices, FLOAT epsilon, out int dimension, out int[] indices) {
            ...
            if (information.Dimension == 1) {
                FLOAT[] array2 = new FLOAT[count];
                FVector3 origin = information.Origin;
                FVector3 vector = information.Direction[0];
                for (int i = 0; i < count; i++) {
                    FVector3 value = vertices[i] - origin;
                    array2[i] = vector.Dot(value);
                }
            ...
            if (information.Dimension == 2) {
                FVector2[] array3 = new FVector2[count];
                FVector3 origin2 = information.Origin;
                FVector3 vector2 = information.Direction[0];
                FVector3 vector3 = information.Direction[1];
                for (int j = 0; j < count; j++) {
                    FVector3 value2 = vertices[j] - origin2;
                    ref FVector2 reference = ref array3[j];
                    reference = new FVector2(vector2.Dot(value2), vector3.Dot(value2));
                }
            ...
            dimension = 3;
            FVector3[] array4 = new FVector3[count];
            FVector3 min = information.Min;
            FLOAT num = 1f / information.MaxRange;
            for (int k = 0; k < count; k++) {
                ref FVector3 reference2 = ref array4[k];
