...
namespace FixedMath {
    public static class Approximation {
        ...
        /// </summary>
        public static Box2D GaussPointsFit2(IList<FVector2> points) {
            Box2D result = new Box2D(FVector2.Zero, FVector2.Right, FVector2.Up, FVector2.One);
            ...
            for (int j = 0; j < count; j++) {
                FVector2 vector = points[j] - result.center;
        ...
        /// </summary>
        internal static bool HeightLineFit2(IList<FVector2> points, out FLOAT a, out FLOAT b) {
            FLOAT num = 0f;
            FLOAT num2 = 0f;
            FLOAT num3 = 0f;
            FLOAT num4 = 0f;
        ...
        /// </summary>
        public static Line2D LeastSquaresLineFit2(IList<FVector2> points) {
            ...
            FLOAT num4 = 0f;
            for (int i = 0; i < count; i++) {
                FVector2 vector = points[i] - result.origin;
                num2 += vector.x * vector.x;
                num3 += vector.x * vector.y;
                num4 += vector.y * vector.y;
            }
        ...
        /// A set must contain at least one point!
        /// </summary>
        public static Box GaussPointsFit3(IList<FVector3> points) {
            Box result = new Box(FVector3.Zero, FVector3.Right, FVector3.Up, FVector3.Forward, FVector3.One);
            int count = points.Count;
            result.center = points[0];
            for (int i = 1; i < count; i++) {
                result.center += points[i];
            ...
            FLOAT num7 = 0f;
            for (int j = 0; j < count; j++) {
                FVector3 vector = points[j] - result.center;
                num2 += vector.x * vector.x;
                num3 += vector.x * vector.y;
        ...
        /// Producing a line using least-squares fitting. A set must contain at least one point!
        /// </summary>
        public static Line LeastsSquaresLineFit3(IList<FVector3> points) {
            ...
            FLOAT num7 = 0f;
            for (int i = 0; i < count; i++) {
                FVector3 vector = points[i] - result.origin;
                num2 += vector.x * vector.x;
                num3 += vector.x * vector.y;
                num4 += vector.x * vector.z;
                num5 += vector.y * vector.y;
        ...
        /// </summary>
        internal static bool HeightPlaneFit3(IList<FVector3> points, out FLOAT a, out FLOAT b, out FLOAT c) {
            FLOAT num = 0f;
            FLOAT num2 = 0f;
            FLOAT num3 = 0f;
            FLOAT num4 = 0f;
        ...
        /// </summary>
        public static Plane LeastSquaresPlaneFit3(IList<FVector3> points) {
            ...
            FLOAT num7 = 0f;
            for (int i = 0; i < count; i++) {
                FVector3 vector = points[i] - point;
                num2 += vector.x * vector.x;
                num3 += vector.x * vector.y;
                num4 += vector.x * vector.z;
                num5 += vector.y * vector.y;
            ...
            FVector3 normal = eigenData.GetEigenvector3(2);
