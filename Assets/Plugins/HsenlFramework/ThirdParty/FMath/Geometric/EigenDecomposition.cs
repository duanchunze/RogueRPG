    ...
    public static class EigenDecomposition {
        ...
        private static void Tridiagonal3(FLOAT[] diagonal, FLOAT[] subdiagonal, FLOAT[,] matrix, out bool isRotation) {
            ...
            if (FMath.Abs(num3) > 1E-05f) {
                FLOAT num7 = FMath.Sqrt(num2 * num2 + num3 * num3);
                FLOAT num8 = 1f / num7;
                num2 *= num8;
                num3 *= num8;
        ...
        private static bool QLAlgorithm(FLOAT[] diagonal, FLOAT[] subdiagonal, FLOAT[,] matrix) {
            ...
            for (int i = 0; i < num2; i++) {
                ...
                for (j = 0; j < num; j++) {
                    int k;
                    for (k = i; k <= num2 - 2; k++) {
                        FLOAT num3 = FMath.Abs(diagonal[k]) + FMath.Abs(diagonal[k + 1]);
                        if (FMath.Abs(subdiagonal[k]) + num3 == num3) {
                            break;
                        }
                    }
                    ...
                    FLOAT num5 = FMath.Sqrt(num4 * num4 + 1f);
                    num4 = ((!(num4 < 0f))
                    ...
                    for (int num9 = k - 1; num9 >= i; num9--) {
                        ...
                        if (FMath.Abs(num10) >= FMath.Abs(num4)) {
                            num7 = num4 / num10;
                            num5 = FMath.Sqrt(num7 * num7 + 1f);
