...
namespace Luban.Editor {
    [CreateAssetMenu(menuName = "Luban/ExportSetting", fileName = "LubanExportSetting")]
    public class LubanExportSetting : ScriptableObject {
        #region 生命周期
        ...
        [Required]
        [Command("-d")]
        [Sirenix.OdinInspector.FilePath(Extensions = "xml")]
        ...
        private ValueDropdownList<string> service_dropdown {
            get {
                if (this._service_dropdown != null) {
                    return this._service_dropdown;
                }

                this._service_dropdown = new ValueDropdownList<string>();

                if (!File.Exists(this.define_xml)) {
                    return this._service_dropdown;
                }

                var doc = new XmlDocument();
                doc.Load(this.define_xml);
                ...
                for (int i = 0; i < nodes.Count; i++) {
                    ...
                    this._service_dropdown.Add(name);
                }
        ...
        private bool _CheckGenType(string which_key_word) {
            foreach (GenTypes type in Enum.GetValues(typeof(GenTypes))) {
                ...
                if (!this.gen_types.HasFlag(type)) {
                    continue;
                }
        ...
        [Button("生成")]
        public void Gen() {
            GenUtils.Gen(this._GetCommand(), this.before_gen, this.after_gen);
        }
