...
namespace Hsenl.View {
    public class UIAbilityInfo : UI<UIAbilityInfo> {
        ...
        public void FillIn(Ability ability) {
            ...
            this.cdText.text = LocalizationHelper.GetStringOrDefault(LocalizationKey.cd) + $" {config.Cooldown}";
            this.manaCostText.text = LocalizationHelper.GetStringOrDefault(LocalizationKey.mana_cost) + $" {config.ManaCost}";
                            ...
                            default: {
                                ...
                                foreach (var damageFormula in harmFormula.DamageFormulas) {
                                    harmStr += LocalizationHelper.GetStringOrDefault(damageFormula.Type) +
                                               $"(+{damageFormula.Fix} *{damageFormula.Pct})";
                                }
