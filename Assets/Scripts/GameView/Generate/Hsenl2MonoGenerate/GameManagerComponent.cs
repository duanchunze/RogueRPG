...
namespace Hsenl.Mono {
    public class GameManagerComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.GameManager> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.GameManager _hsenlComponent;
    
        public Hsenl.GameManager HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.GameManager;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(GameManagerComponent))]
    public class GameManagerComponentEditor : OdinEditor {
        private GameManagerComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (GameManagerComponent)this.target;
        }
