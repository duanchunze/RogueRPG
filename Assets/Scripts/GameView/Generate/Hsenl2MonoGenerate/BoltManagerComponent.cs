...
namespace Hsenl.Mono {
    public class BoltManagerComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.BoltManager> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.BoltManager _hsenlComponent;
    
        public Hsenl.BoltManager HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.BoltManager;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(BoltManagerComponent))]
    public class BoltManagerComponentEditor : OdinEditor {
        private BoltManagerComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (BoltManagerComponent)this.target;
        }
