...
namespace Hsenl.Mono {
    public class NetworkClientComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.Network.NetworkClient> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.Network.NetworkClient _hsenlComponent;
    
        public Hsenl.Network.NetworkClient HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.Network.NetworkClient;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(NetworkClientComponent))]
    public class NetworkClientComponentEditor : OdinEditor {
        private NetworkClientComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (NetworkClientComponent)this.target;
        }
