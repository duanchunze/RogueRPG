...
namespace Hsenl.Mono {
    public class ControlTriggerComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.ControlTrigger> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.ControlTrigger _hsenlComponent;
    
        public Hsenl.ControlTrigger HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.ControlTrigger;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(ControlTriggerComponent))]
    public class ControlTriggerComponentEditor : OdinEditor {
        private ControlTriggerComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (ControlTriggerComponent)this.target;
        }
