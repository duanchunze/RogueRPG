...
namespace Hsenl.Mono {
    public class ScopeComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.Scope> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.Scope _hsenlComponent;
    
        public Hsenl.Scope HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.Scope;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(ScopeComponent))]
    public class ScopeComponentEditor : OdinEditor {
        private ScopeComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (ScopeComponent)this.target;
        }
