...
namespace Hsenl.Mono {
    public class NavMeshSurfaceComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.NavMeshSurface> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.NavMeshSurface _hsenlComponent;
    
        public Hsenl.NavMeshSurface HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.NavMeshSurface;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(NavMeshSurfaceComponent))]
    public class NavMeshSurfaceComponentEditor : OdinEditor {
        private NavMeshSurfaceComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (NavMeshSurfaceComponent)this.target;
        }
