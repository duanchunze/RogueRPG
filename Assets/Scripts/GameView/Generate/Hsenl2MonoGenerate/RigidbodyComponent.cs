...
namespace Hsenl.Mono {
    public class RigidbodyComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.Rigidbody> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.Rigidbody _hsenlComponent;
    
        public Hsenl.Rigidbody HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.Rigidbody;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(RigidbodyComponent))]
    public class RigidbodyComponentEditor : OdinEditor {
        private RigidbodyComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (RigidbodyComponent)this.target;
        }
