...
namespace Hsenl.Mono {
    public class NavMeshAgentComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.NavMeshAgent> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.NavMeshAgent _hsenlComponent;
    
        public Hsenl.NavMeshAgent HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.NavMeshAgent;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(NavMeshAgentComponent))]
    public class NavMeshAgentComponentEditor : OdinEditor {
        private NavMeshAgentComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (NavMeshAgentComponent)this.target;
        }
