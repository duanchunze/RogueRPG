...
namespace Hsenl.Mono {
    public class HarmableComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.Harmable> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.Harmable _hsenlComponent;
    
        public Hsenl.Harmable HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.Harmable;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(HarmableComponent))]
    public class HarmableComponentEditor : OdinEditor {
        private HarmableComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (HarmableComponent)this.target;
        }
