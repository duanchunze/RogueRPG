...
namespace Hsenl.Mono {
    public class BoltComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.Bolt> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.Bolt _hsenlComponent;
    
        public Hsenl.Bolt HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.Bolt;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(BoltComponent))]
    public class BoltComponentEditor : OdinEditor {
        private BoltComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (BoltComponent)this.target;
        }
