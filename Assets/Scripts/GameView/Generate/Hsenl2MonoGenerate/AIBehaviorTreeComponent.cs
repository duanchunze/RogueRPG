...
namespace Hsenl.Mono {
    public class AIBehaviorTreeComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.AIBehaviorTree> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.AIBehaviorTree _hsenlComponent;
    
        public Hsenl.AIBehaviorTree HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.AIBehaviorTree;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(AIBehaviorTreeComponent))]
    public class AIBehaviorTreeComponentEditor : OdinEditor {
        private AIBehaviorTreeComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (AIBehaviorTreeComponent)this.target;
        }
