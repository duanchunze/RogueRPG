...
namespace Hsenl.Mono {
    public class PickableComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.Pickable> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.Pickable _hsenlComponent;
    
        public Hsenl.Pickable HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.Pickable;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(PickableComponent))]
    public class PickableComponentEditor : OdinEditor {
        private PickableComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (PickableComponent)this.target;
        }
