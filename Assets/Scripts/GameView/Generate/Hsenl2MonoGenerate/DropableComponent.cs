...
namespace Hsenl.Mono {
    public class DropableComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.Dropable> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.Dropable _hsenlComponent;
    
        public Hsenl.Dropable HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.Dropable;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(DropableComponent))]
    public class DropableComponentEditor : OdinEditor {
        private DropableComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (DropableComponent)this.target;
        }
