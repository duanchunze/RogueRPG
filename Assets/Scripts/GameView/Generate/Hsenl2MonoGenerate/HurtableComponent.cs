...
namespace Hsenl.Mono {
    public class HurtableComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.Hurtable> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.Hurtable _hsenlComponent;
    
        public Hsenl.Hurtable HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.Hurtable;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(HurtableComponent))]
    public class HurtableComponentEditor : OdinEditor {
        private HurtableComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (HurtableComponent)this.target;
        }
