...
namespace Hsenl.Mono {
    public class CardManagerComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.CardManager> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.CardManager _hsenlComponent;
    
        public Hsenl.CardManager HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.CardManager;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(CardManagerComponent))]
    public class CardManagerComponentEditor : OdinEditor {
        private CardManagerComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (CardManagerComponent)this.target;
        }
