...
namespace Hsenl.Mono {
    public class NetworkServerComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.Network.NetworkServer> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.Network.NetworkServer _hsenlComponent;
    
        public Hsenl.Network.NetworkServer HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.Network.NetworkServer;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(NetworkServerComponent))]
    public class NetworkServerComponentEditor : OdinEditor {
        private NetworkServerComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (NetworkServerComponent)this.target;
        }
