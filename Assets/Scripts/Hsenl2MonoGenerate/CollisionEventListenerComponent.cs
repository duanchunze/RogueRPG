...
namespace Hsenl.Mono {
    public class CollisionEventListenerComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.CollisionEventListener> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.CollisionEventListener _hsenlComponent;
    
        public Hsenl.CollisionEventListener HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.CollisionEventListener;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(CollisionEventListenerComponent))]
    public class CollisionEventListenerComponentEditor : OdinEditor {
        private CollisionEventListenerComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (CollisionEventListenerComponent)this.target;
        }
