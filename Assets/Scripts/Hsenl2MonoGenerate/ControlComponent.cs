...
namespace Hsenl.Mono {
    public class ControlComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.Control> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.Control _hsenlComponent;
    
        public Hsenl.Control HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.Control;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(ControlComponent))]
    public class ControlComponentEditor : OdinEditor {
        private ControlComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (ControlComponent)this.target;
        }
