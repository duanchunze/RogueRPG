...
namespace Hsenl.Mono {
    public class BodiedComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.Bodied> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.Bodied _hsenlComponent;
    
        public Hsenl.Bodied HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.Bodied;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(BodiedComponent))]
    public class BodiedComponentEditor : OdinEditor {
        private BodiedComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (BodiedComponent)this.target;
        }
