...
namespace Hsenl.Mono {
    public class TcpServerComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.Network.TcpServer> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.Network.TcpServer _hsenlComponent;
    
        public Hsenl.Network.TcpServer HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.Network.TcpServer;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(TcpServerComponent))]
    public class TcpServerComponentEditor : OdinEditor {
        private TcpServerComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (TcpServerComponent)this.target;
        }
