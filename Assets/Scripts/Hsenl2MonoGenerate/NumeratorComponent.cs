...
namespace Hsenl.Mono {
    public class NumeratorComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.Numerator> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.Numerator _hsenlComponent;
    
        public Hsenl.Numerator HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.Numerator;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(NumeratorComponent))]
    public class NumeratorComponentEditor : OdinEditor {
        private NumeratorComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (NumeratorComponent)this.target;
        }
