...
namespace Hsenl.Mono {
    public class ActorManagerComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.ActorManager> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.ActorManager _hsenlComponent;
    
        public Hsenl.ActorManager HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.ActorManager;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(ActorManagerComponent))]
    public class ActorManagerComponentEditor : OdinEditor {
        private ActorManagerComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (ActorManagerComponent)this.target;
        }
