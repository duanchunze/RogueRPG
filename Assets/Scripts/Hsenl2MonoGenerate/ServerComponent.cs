...
namespace Hsenl.Mono {
    public class ServerComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.Server> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.Server _hsenlComponent;
    
        public Hsenl.Server HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.Server;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(ServerComponent))]
    public class ServerComponentEditor : OdinEditor {
        private ServerComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (ServerComponent)this.target;
        }
