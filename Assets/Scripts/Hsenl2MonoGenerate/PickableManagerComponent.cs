...
namespace Hsenl.Mono {
    public class PickableManagerComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.PickableManager> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.PickableManager _hsenlComponent;
    
        public Hsenl.PickableManager HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.PickableManager;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(PickableManagerComponent))]
    public class PickableManagerComponentEditor : OdinEditor {
        private PickableManagerComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (PickableManagerComponent)this.target;
        }
