...
namespace Hsenl.Mono {
    public class CardBackpackComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.CardBackpack> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.CardBackpack _hsenlComponent;
    
        public Hsenl.CardBackpack HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.CardBackpack;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(CardBackpackComponent))]
    public class CardBackpackComponentEditor : OdinEditor {
        private CardBackpackComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (CardBackpackComponent)this.target;
        }
