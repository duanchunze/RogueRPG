...
namespace Hsenl.Mono {
    public class TimeLineComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.TimeLine> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.TimeLine _hsenlComponent;
    
        public Hsenl.TimeLine HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.TimeLine;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(TimeLineComponent))]
    public class TimeLineComponentEditor : OdinEditor {
        private TimeLineComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (TimeLineComponent)this.target;
        }
