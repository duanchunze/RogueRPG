...
namespace Hsenl.Mono {
    public class PickerComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.Picker> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.Picker _hsenlComponent;
    
        public Hsenl.Picker HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.Picker;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(PickerComponent))]
    public class PickerComponentEditor : OdinEditor {
        private PickerComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (PickerComponent)this.target;
        }
