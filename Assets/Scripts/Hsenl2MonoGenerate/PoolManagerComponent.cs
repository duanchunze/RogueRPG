...
namespace Hsenl.Mono {
    public class PoolManagerComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.PoolManager> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.PoolManager _hsenlComponent;
    
        public Hsenl.PoolManager HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.PoolManager;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(PoolManagerComponent))]
    public class PoolManagerComponentEditor : OdinEditor {
        private PoolManagerComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (PoolManagerComponent)this.target;
        }
