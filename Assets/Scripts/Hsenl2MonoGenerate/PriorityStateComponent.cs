...
namespace Hsenl.Mono {
    public class PriorityStateComponent : MonoBehaviour, IHsenlComponentReference<Hsenl.PriorityState> {
        [ReadOnly]
        public int hsenlComponentInstanceId;
    
        [SerializeField]
        private Hsenl.PriorityState _hsenlComponent;
    
        public Hsenl.PriorityState HsenlComponent => this._hsenlComponent;
        ...
        void IHsenlComponentReference.SetFrameworkReference(Component reference) {
            this._hsenlComponent = reference as Hsenl.PriorityState;
            this.hsenlComponentInstanceId = this._hsenlComponent?.InstanceId??0;
        }
    }
    
    #if UNITY_EDITOR
    [CustomEditor(typeof(PriorityStateComponent))]
    public class PriorityStateComponentEditor : OdinEditor {
        private PriorityStateComponent _t;

        protected override void OnEnable() {
            base.OnEnable();
            this._t = (PriorityStateComponent)this.target;
        }
