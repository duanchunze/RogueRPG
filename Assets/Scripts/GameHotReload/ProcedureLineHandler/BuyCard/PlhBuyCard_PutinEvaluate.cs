...
//     public class PlhBuyCard_PutinEvaluate : AProcedureLineHandler<PliBuyCardForm> {
...
//                         item.slot = cardBar.PutinCardEvaluate(item.target, item.slot as CardBarSlot);
//                         if (item.slot == null) {
//                             var cardBackpack = cardBar.MainBodied.FindBodiedInIndividual<CardBackpack>();
//                             item.slot = cardBackpack.PutinCardEvaluate(item.target, item.slot as CardBackpackSlot);
//                             if (item.slot == null) {
//                                 // 都失败直接跳出, 购买失败
